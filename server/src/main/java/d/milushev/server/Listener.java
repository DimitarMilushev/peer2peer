...
import main.java.d.milushev.Processor;
...
public class Listener implements Runnable, AutoCloseable {
    ...
    public Listener(int port, Processor processor) throws IOException {
        this.port = port;
    ...
    private void handleWrite(SelectionKey key) throws ServerException {
        try {
            if (key.channel() instanceof SocketChannel clientChannel) {
                final var response = processor.getResponseForChannel(clientChannel);
                if (response == null) {
                    return;
                }
                ...
                buffer.put(response.get().toString().getBytes(StandardCharsets.UTF_8));
    ...
    private void handleRead(SelectionKey key) throws ServerException {
        if (key.channel() instanceof SocketChannel clientChannel) {
            ...
            try {
                ...
                processor.enqueue(new Request(sb.toString(), null, clientChannel));
        ...
        throw new ServerException("Failed to process WRITE operation", new InvalidConnectionHandling("Invalid channel was opened for READ operation"));
    }
    ...
    private void closeClientChannel(SocketChannel channel) throws ServerException {
        try {
            ...
            // TODO: sync somehow
            processor.enqueue(new Request(CloseUserCommand.NAME, null, channel));
            Thread.sleep(500);
